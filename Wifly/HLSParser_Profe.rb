...
# static initcialitzation code
command($ser, "$$$", "CMD\r\n") #switch to command mode
p("$$$")
command($ser, "set wlan ext_antenna 1\r", "<4\.41>")    # set ext antenna and wait for prompt
...
def confWifly
    ...
    command('set option format 1', 'AOK') #mostra per pantalla tant bon punt arriba el header HTML  --> no es optim ja que te un timeout
    # bona solució és fer --> set broadcast interval 1 (...after interval or 0, undocumented)
end
